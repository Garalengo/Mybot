 ...
 * Версия: 7.0 (ФИНАЛЬНАЯ, ИСПРАВЛЕНА ОЧИСТКА КАНАЛОВ)
 * ============================================================
 */
...
// ==================== ФУНКЦИЯ ОЧИСТКИ ИМЕНИ КАНАЛА (ИСПРАВЛЕНА) ====================
...
bot.on('callback_query', async (msg) => {
    ...
    // ============================================================
    //  АДМИН-ПАНЕЛЬ (ОБРАБОТКА ВСЕХ КНОПОК)
    // ============================================================
    if (data[0] === 'admin') {
        if (!ADMIN_IDS.includes(userId)) {
        ...
        if (action === 'withdraws') {
            ...
            db.get(`SELECT COUNT(*) AS count FROM withdraws WHERE status = 0`, (err, row) => {
                ...
                if (total === 0) {
                    bot.sendMessage(chatId, '✅ Нет новых заявок на выплату.').catch(() => {});
                    return;
                }
        ...
        if (action === 'editchannels') {
            db.all(
                ...
                (err, rows) => {
                    if (err || !rows || rows.length === 0) {
                        bot.sendMessage(chatId, '📭 Нет активных каналов для подписки.').catch(() => {});
                        return;
                    }
                    const keyboard = rows.map(row => [
        ...
        if (action === 'delete') {
            ...
            bot.sendMessage(chatId, '🗑 Канал успешно удалён.').catch(() => {});
            return;
        }
