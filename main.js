...
bot.on('callback_query', async (msg) => {
    ...
    switch (data[0]) {
        ...
        // ----- 1. ЗАЯВКИ НА ВЫПЛАТУ -----
        case 'admin_withdraws': {
            if (!ADMIN_IDS.includes(userId)) break;

            if (data[1] === 'skip') withdrawOffset++;
            else if (data[1] === 'reset') withdrawOffset = 0;
        ...
        // ----- 7. РЕДАКТИРОВАТЬ КАНАЛ (ПРОСМОТР) -----
        case 'admin_edit': {
            if (!ADMIN_IDS.includes(userId)) break;
            const id = parseInt(data[1]);
