...
// Добавлена система заданий (tasks)
...
// Создание таблиц (включая tasks и user_tasks)
...
const adminTask = new Map(); // для создания задания
...
// ========== ФУНКЦИЯ ПРОВЕРКИ ПОДПИСКИ (общая) ==========
...
bot.onText(new RegExp(menu_keyboard.keyboard[3][1]), async (msg) => {
    const chatId = msg.chat.id;
...
// ========== ЗАДАНИЯ (главная кнопка) ==========
bot.onText('📋 Задания', async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const subscriptions = await checkSubscriptions(userId);
    if (subscriptions === true) {
        // Получаем активные задания, которые пользователь ещё не выполнил
        db.all(`
            SELECT t.*
            FROM tasks t
        ...
        `, [userId], (err, rows) => {
            if (err) {
                log(err);
                bot.sendMessage(chatId, '❌ Ошибка загрузки заданий.').catch(() => {});
                return;
            }
            ...
            // Формируем сообщение со списком заданий
            let text = '📋 <b>Доступные задания:</b>\n\n';
            let keyboard = [];
            rows.forEach(row => {
                text += `🔹 <b>${row.channel}</b> – награда: ${row.reward} ${config.currency}\n`;
                keyboard.push([{
                    text: `✅ Проверить подписку на ${row.channel}`,
                    callback_data: `checktask_${row.id}`
                }]);
            });
...
// ========== ПРОМОКОДЫ (без проверки наличия) ==========
...
bot.on('message', async (msg) => {
    if (msg) {
        const userId = msg.from.id;
        const chatId = msg.chat.id;
        if (msg.text && (msg.text.startsWith('/') || msg.text === '❌ Отменить')) return;
        ...
        if (subscriptions === true) {
            ...
            if (withdraws.has(userId)) {
                if (!withdraws.get(userId).amount) {
                    if (isNaN(msg.text)) {
                        bot.sendMessage(chatId, "❌ Нужно ввести число. Операция отменена.", {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        withdraws.delete(userId);
                        return;
                    ...
                    if (amount < minAmount) {
                        bot.sendMessage(chatId, "💰 | Мин. сумма: " + minAmount + ". Попробуйте снова или нажмите ❌ Отменить", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    db.get(`SELECT * FROM users WHERE chatId = ?`, [userId], (err, row) => {
                        ...
                        if (amount > row.balance) {
                            bot.sendMessage(chatId, "💰 | Макс. сумма: " + row.balance + ". Операция отменена.", {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            withdraws.delete(userId);
                            return;
                        ...
                        bot.sendMessage(chatId, "💰 | Введите реквизиты для вывода (Карта, СБП, Криптокошелёк и др.):", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                    });
                    ...
                    if (msg.text !== withdraws.get(userId).wallet) {
                        bot.sendMessage(chatId, "❌ Реквизиты не совпадают. Попробуйте снова или нажмите ❌ Отменить", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        withdraws.delete(userId);
                        return;
            ...
            // ---------- ДОБАВЛЕНИЕ КАНАЛА ----------
                ...
                if (!addchannel.get(userId).hours) {
                    ...
                    if (hours < 1) {
                        bot.sendMessage(chatId, "💰 | Мин. срок: от 1 часа. Попробуйте снова или нажмите ❌ Отменить", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    db.get(`SELECT * FROM users WHERE chatId = ?`, [userId], (err, row) => {
                        ...
                        if (hours > row.balance * priceperhour) {
                            bot.sendMessage(chatId, `❌ Недостаточный баланс: ${hours} * ${priceperhour} > ${row.balance}`, {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            addchannel.delete(userId);
                            return;
                        ...
                        bot.sendMessage(chatId, "💰 | Отправьте @username канала.", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                    });
                } else if (!addchannel.get(userId).channel) {
                    const channelClean = msg.text.trim().replace(/[@https:\/\/t.me\/]/g, '');
                    const chat = await bot.getChat('@' + channelClean).catch(() => {});
                    if (!chat) {
                        bot.sendMessage(chatId, "❌ Канал не найден. Убедитесь, что бот администратор. Операция отменена.", {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        addchannel.delete(userId);
                        return;
                    }
                    const chatmember = await bot.getChatMember('@' + channelClean, userId).catch(() => {});
                    if (!chatmember) {
                        bot.sendMessage(chatId, "❌ Бот не администратор канала. Операция отменена.", {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        addchannel.delete(userId);
                        return;
                    ...
                    db.get(`SELECT count(*) as count FROM subscriptions WHERE datetime(creationDate, '+' || hours || ' hours') > datetime('now')`, async (err, row) => {
                        ...
                        if (row.count < maxaddedrequiredchannels) {
                            ...
                            bot.sendMessage(chatId, "❌ Максимальное количество каналов достигнуто.", {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            addchannel.delete(userId);
                        }
                ...
                if (!orderbroadcasts.get(userId).auditory) {
                    ...
                    db.get(`SELECT balance FROM users WHERE chatId = ?`, [userId], (err, row) => {
                        ...
                        if (row.balance < auditory * priceperuser) {
                            bot.sendMessage(chatId, `❌ Недостаточно баланса. Доступно: ${row.balance}, требуется: ${auditory * priceperuser}`, {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            orderbroadcasts.delete(userId);
                            return;
                ...
                switch (adminfuncs.get(userId).func) {
                    case "changebalance": {
                        ...
                        if (parts.length < 2 || isNaN(parts[0]) || isNaN(parts[1])) {
                            bot.sendMessage(chatId, "❌ Неверный формат. Нужно: ID сумма (например, 123456 -10.5)", {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            adminfuncs.delete(userId);
                            return;
            ...
            // ---------- ПРОСМОТР РЕФЕРАЛОВ (АДМИН) ----------
                ...
                if (!isNaN(msg.text)) {
                    db.all(`SELECT * FROM users WHERE referer = ? LIMIT 100`, [parseInt(msg.text)], (err, rows) => {
                        if (err) { log(err); return; }
                        if (rows.length === 0) {
                            bot.sendMessage(chatId, "Рефералы не найдены", {
                                reply_markup: menu_keyboard
                            }).catch(() => {});
                            return;
                        }
                    ...
                    bot.sendMessage(chatId, "Нужно ввести число.", {
                        reply_markup: menu_keyboard
                    }).catch(() => {});
                }
            }
            // ---------- АКТИВАЦИЯ ПРОМОКОДА ----------
                ...
                db.get(`SELECT * FROM promocodes p WHERE p.hash = ? AND p.hash NOT IN (SELECT hash FROM promocodeactivations WHERE userId = ?) AND p.activations > (SELECT count(*) FROM promocodeactivations WHERE hash = p.hash)`, [msg.text, userId], async (err, row) => {
                    if (err || !row) {
                        bot.sendMessage(chatId, "Промокод не найден или уже активирован", {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        return;
                    }
                    ...
                    await bot.sendMessage(userId, '✅ Промокод активирован: <b>+' + row.sum.toFixed(2) + ' ' + config.currency + '</b>', {
                        parse_mode: 'HTML',
                        reply_markup: menu_keyboard
                    }).catch(() => {});
                    await bot.sendMessage(admin[0], '⚠️ Промокод <b>' + msg.text + '</b> активирован пользователем <b>' + userId + '</b>', {parse_mode: 'HTML'}).catch((err) => { log(err.message); });
                });
            }
            // ---------- СОЗДАНИЕ ПРОМОКОДА (АДМИН) ----------
                ...
                if (!promo.sum) {
                    ...
                    db.run(`INSERT INTO promocodes (hash, activations, sum) VALUES (?, ?, ?)`, [promo.hash, promo.activations, promo.sum], async (err) => {
                        if (err) { log(err); return; }
                        await bot.sendMessage(userId, `Промокод на сумму ${promo.sum.toFixed(2)} ${config.currency} и ${promo.activations} активаций создан: <code>${promo.hash}</code>`, {
                            parse_mode: 'HTML',
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        adminpromocode.delete(userId);
            ...
            // ---------- СОЗДАНИЕ ЗАДАНИЯ (АДМИН) ----------
                ...
                if (!taskData.channel) {
                    ...
                    if (!channel) {
                        bot.sendMessage(chatId, "❌ Некорректный username канала. Попробуйте снова или /cancel", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    // Проверяем, существует ли канал (проверка через бота)
                    try {
                        await bot.getChat('@' + channel);
                    } catch (e) {
                        bot.sendMessage(chatId, "❌ Канал не найден. Убедитесь, что бот администратор и username правильный.", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    ...
                    bot.sendMessage(chatId, "💰 | Введите сумму вознаграждения (в " + config.currency + "):", {
                        reply_markup: cancel_keyboard
                    }).catch(() => {});
                } else if (!taskData.reward) {
                    if (isNaN(msg.text)) {
                        bot.sendMessage(chatId, "❌ Нужно ввести число. Попробуйте снова или /cancel", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    ...
                    if (reward <= 0) {
                        bot.sendMessage(chatId, "❌ Сумма должна быть больше 0. Попробуйте снова.", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                        return;
                    }
                    ...
                    db.run(`INSERT INTO tasks (channel, reward, created_by) VALUES (?, ?, ?)`, [taskData.channel, reward, userId], function(err) {
                        if (err) {
                            log(err);
                            bot.sendMessage(chatId, "❌ Ошибка сохранения задания.").catch(() => {});
                            return;
                        }
                        bot.sendMessage(chatId, `✅ Задание для канала @${taskData.channel} с наградой ${reward} ${config.currency} создано!`, {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        adminTask.delete(userId);
                    });
...
bot.on('callback_query', async (msg) => {
        ...
        switch (data[0]) {
            case 'withdraw': {
                db.get(`SELECT * FROM users WHERE chatId = ?`, [userId], (err, row) => {
                    ...
                    if (row.balance < minAmount) {
                        bot.sendMessage(chatId, "❌ Мин. сумма вывода: " + minAmount, {
                            reply_markup: menu_keyboard
                        }).catch(() => {});
                        return;
                    }
                    withdraws.set(userId, {amount: undefined, wallet: undefined});
                    bot.sendMessage(chatId, "💰 | Введите сумму от " + minAmount + " до " + row.balance + ":", {
                        reply_markup: cancel_keyboard
                    }).catch(() => {});
                });
                break;
            ...
            case 'withdraws': {
                if (admin.includes(userId)) {
                    ...
                    db.get(`SELECT count(*) as count FROM withdraws WHERE status = 0`, (err2, row2) => {
                        if (err2) { log(err2); return; }
                        ...
                        db.get(`SELECT * FROM withdraws WHERE status = 0 LIMIT 1 OFFSET ?`, [current_withdraw_offset], (err1, row1) => {
                            if (err1) { log(err1); return; }
                            if (row1)
            ...
            case 'addchannel': {
                db.get(`SELECT count(*) as count FROM subscriptions WHERE datetime(creationDate, '+' || hours || ' hours') > datetime('now')`, (err, row) => {
                    ...
                    if (row.count < maxaddedrequiredchannels) {
                        ...
                        bot.sendMessage(chatId, "💰 | Отправьте срок (в часах):", {
                            reply_markup: cancel_keyboard
                        }).catch(() => {});
                    } else {
            ...
            case 'orderbroadcast': {
                if (data[1]) {
                    ...
                    bot.sendMessage(chatId, "⚠️ Введите количество человек.\nЦена: " + priceperuser + " " + config.currency + " за чел.", {
                        reply_markup: cancel_keyboard
                    }).catch(() => {});
                }
                break;
            ...
            case 'editchannels': {
                if (admin.includes(userId)) {
                    db.all(`SELECT * FROM subscriptions WHERE datetime(creationDate, '+' || hours || ' hours') > datetime('now')`, (err, rows) => {
                        if (err) { log(err); return; }
                        if (!rows) return;
                        let keyboard = rows.map(row => [{ text: row.title, callback_data: 'editchannel_' + row.id }]);
            ...
            case 'reftop': {
                ...
                const query = `
                    SELECT u1.id, u1.firstName, u1.username, u1.referer,
            ...
            case 'adminreferals': {
                ...
                await bot.sendMessage(userId, 'Введите ID пользователя:', {
                    reply_markup: cancel_keyboard
                });
                break;
            }
            ...
            case 'createtask': {
                if (admin.includes(userId)) {
                    ...
                    await bot.sendMessage(chatId, '📝 Введите username канала (без @) для задания:', {
                        reply_markup: cancel_keyboard
                    });
                }
                break;
            ...
            case 'checktask': {
                ...
                db.get(`SELECT * FROM user_tasks WHERE user_id = ? AND task_id = ?`, [userId, taskId], (err, row) => {
                    ...
                    db.get(`SELECT * FROM tasks WHERE id = ? AND active = 1`, [taskId], async (err, task) => {
                        ...
                        const chatMember = await bot.getChatMember(channel, userId).catch(() => {});
                        ...
                        // Если подписан – начисляем награду
                        db.run(`UPDATE users SET balance = balance + ? WHERE chatId = ?`, [task.reward, userId], function(err) {
                            if (err) {
                                log(err);
                                bot.sendMessage(chatId, "❌ Ошибка начисления баланса.").catch(() => {});
                                return;
                            }
                            // Отмечаем выполнение
    ...
    bot.answerCallbackQuery(msg.id).catch((err) => { log(err.message); });
});
...
function broadcastMessageConfirm(msg, userId, order = false) {
    ...
    if (order) {
        msg_b_form.reply_markup.inline_keyboard.push([
            { text: 'Подтвердить', callback_data: 'orderbroadcast_confirm' },
            { text: 'Отклонить', callback_data: 'orderbroadcast_decline' }
        ]);
    } else {
        msg_b_form.reply_markup.inline_keyboard.push([
            { text: 'Подтвердить', callback_data: 'broadcast_confirm' },
