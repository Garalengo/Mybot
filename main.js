...
// Добавлена обязательная подписка, кнопка отмены, проверка промокодов
...
// Команда /cancel и кнопка "❌ Отменить" (обрабатывается отдельно)
...
// ========== ОСНОВНАЯ ФУНКЦИЯ СТАРТ С ПРОВЕРКОЙ ПОДПИСКИ ==========
...
// ========== ПРОМОКОДЫ С ПРОВЕРКОЙ НАЛИЧИЯ ==========
bot.onText('🎵 Промокоды', async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const subscriptions = await checkSubscriptions(userId);
    if (subscriptions === true) {
        db.get(`SELECT SUM(p.activations - COALESCE((SELECT COUNT(*) FROM promocodeactivations WHERE hash = p.hash), 0)) AS total FROM promocodes p`, (err, row) => {
            if (err) {
                log(err);
                bot.sendMessage(chatId, '❌ Ошибка проверки промокодов.').catch(() => {});
                return;
            }
            const totalAvailable = row ? row.total : 0;
            if (totalAvailable > 0) {
                bot.sendMessage(chatId, '⭐ Введите промокод для активации:').catch(() => {});
                promocodes.set(userId, {});
            } else {
                const groupLink = config.group || config.chat || 'https://t.me/yourgroup';
                bot.sendMessage(chatId, `На данный момент промокодов не существует. Заходите в нашу группу, чтобы не пропустить промокод: ${groupLink}`).catch(() => {});
            }
        });
    } else {
        bot.sendMessage(chatId, config.subscribemsg.replace('%firstname%', msg.from.first_name), {
