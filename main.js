...
// Версия: 2.0 (исправленная и улучшенная)
// ============================================================
...
// --- Конфигурация ---
const refPrice = parseFloat(config.refPrice) || 10;
const refPrice2 = parseFloat(config.refPrice2) || 5;
const maxaddedrequiredchannels = parseInt(config.maxaddedrequiredchannels) || 5;
const priceperhour = parseFloat(config.priceperhour) || 1;
const minAmount = parseFloat(config.minAmount) || 10;
...
// --- Обновление списка каналов для подписки ---
async function update_channels() {
    let parsed_channels = config.requiredChannels ? config.requiredChannels.split(/[|,]/) : [];
    let required_channels = [];
    for (let i = 0; i < parsed_channels.length; i++) {
        let channel = parsed_channels[i].trim().replace(/[@https:\/\/t.me\/]/g, '');
        if (channel) {
            required_channels.push(['https://t.me/' + channel, '@' + channel, 'Канал ' + (i + 1)]);
        }
    }
...
// --- Вспомогательная функция для таблиц ---
        ...
        if (!row) {
            if (startParam && startParam.length > 0) {
                ...
                if (subscriptions === true) {
                    bot.sendMessage(chatId, config.hellomsg ? config.hellomsg.replace('%firstname%', firstName) : 'Добро пожаловать!', {
                        parse_mode: 'HTML',
                        reply_markup: menu_keyboard
                    }).catch(() => {});
            ...
            const subscriptions = await checkSubscriptions(userId);
            if (subscriptions === true) {
                bot.sendMessage(chatId, config.hellomsg ? config.hellomsg.replace('%firstname%', firstName) : 'С возвращением!', {
                    parse_mode: 'HTML',
                    reply_markup: menu_keyboard
                }).catch(() => {});
                if (row.verified === 0) {
...
bot.onText(/💰 Заработать/, async (msg) => {
    ...
    if (subscriptions === true) {
        db.get(`SELECT *,
                       ...
                       (SELECT COUNT(*) FROM users WHERE referer IN (SELECT chatId FROM users WHERE referer = ?)) AS second_level_referrals,
                       (SELECT COUNT(*) FROM users u3 WHERE u3.referer = u1.chatId) AS first_level_referrals
                FROM users u1 WHERE u1.chatId = ?;`, [userId, userId], (err, result) => {
            if (err) { log(err); return; }
            if (!result) return;
...
// Информация о боте (исправлен индекс)
bot.onText(/📳 Информация о боте/, async (msg) => {
    ...
    `, [twentyFourHoursAgo], (err, result) => {
        ...
        const days = Math.floor((new Date() - new Date(config.startDate || '2026-01-01')) / (24 * 60 * 60 * 1000));
...
📅 <b>Мы работаем уже </b>${days} дней`;
        bot.sendMessage(chatId, response, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: '📙 Канал', url: config.channel || 'https://t.me/yourchannel' },
...
// Промокоды (всегда предлагают ввод)
...
bot.on('message', async (msg) => {
    ...
    const userId = msg.from.id;
    const chatId = msg.chat.id;
    // Игнорируем команды и кнопку отмены
    if (msg.text.startsWith('/') || msg.text === '❌ Отменить' || msg.text === '🎵 Промокоды' || msg.text === '📋 Задания' || msg.text === '📳 Информация о боте' || msg.text === '💰 Заработать' || msg.text === '👇 Личный кабинет' || msg.text === '📙 Продвижение') return;
    ...
    if (addchannel.has(userId)) {
        if (!addchannel.get(userId).hours) {
            ...
            let botMember;
            try {
                botMember = await bot.getChatMember('@' + channelClean, (await bot.getMe()).id);
            } catch (e) {
