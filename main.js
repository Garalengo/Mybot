 ...
 * Версия: 8.0 (ЗАЩИЩЁННАЯ АДМИН-ПАНЕЛЬ)
 * ============================================================
 */
...
const ADMIN_PASSWORD = 'coinref2024';
...
function clearStates(userId) {
    ...
    STATES.adminLogin.delete(userId);
}
...
// Для обычных пользователей добавляем кнопку админки (если админ)
...
const STATES = {
    ...
    adminTask: new Map(),
    adminLogin: new Map() // для хранения состояния входа
};
...
bot.onText(/👑 Админ-панель/, async (msg) => {
    ...
    // Если уже есть состояние логина, удаляем его
    STATES.adminLogin.delete(userId);

    // Запрашиваем логин
    STATES.adminLogin.set(userId, { step: 'login', attempts: 0 });
    await bot.sendMessage(
        chatId,
        '🔐 <b>ВХОД В АДМИН-ПАНЕЛЬ</b>\n\nВведите <b>логин</b> для входа:\n\n(Логин: <code>admin</code>)',
        {
            parse_mode: 'HTML',
...
// Добавляем обработку логина и пароля в основной обработчик сообщений
...
bot.onText(/🏆 Топ/, async (msg) => {
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            text += '\n💪 Продолжайте зарабатывать и поднимайтесь в топе!';
...
• За 1 уровень: <b>${CONFIG.refPrice} ${CONFIG.currency}</b>
• За 2 уровень: <b>${CONFIG.refPrice2} ${CONFIG.currency}</b>
...
<code>https://t.me/${botUsername}?start=${userId}</code>
...
🚀 Делитесь ссылкой и зарабатывайте!`;
            ...
            bot.sendMessage(chatId, text, {
                ...
                reply_markup: {
                    inline_keyboard: [[
                        { text: '📤 Поделиться ссылкой', url: `https://t.me/share/url?url=https%3A//t.me/${botUsername}?start=${userId}` }
                    ]]
                }
            }).catch(() => {});
        }
...
📅 <b>В боте:</b> ${days} дней

➖➖➖➖➖➖➖➖➖
💰 <b>БАЛАНС:</b>
• <b>Текущий:</b> ${balance} ${CONFIG.currency}
• <b>В процессе вывода:</b> ${withdrawing} ${CONFIG.currency}
• <b>Всего заработано:</b> ${withdrawn} ${CONFIG.currency}
        ...
        db.get(
            ...
            (err, row) => {
                ...
                if (count < CONFIG.maxAddedRequiredChannels) {
                    bot.sendMessage(chatId, '📢 <b>ПРОДВИЖЕНИЕ В БОТЕ</b>\n\nЗдесь вы можете:\n• Добавить свой канал для обязательной подписки\n• Заказать рассылку по всем пользователям\n\nВыберите действие:', {
                        parse_mode: 'HTML',
                        reply_markup: {
                            inline_keyboard: [
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            const keyboard = rows.map(row => {
                text += `\n🔹 <b>${row.channel}</b>\n   Награда: ${row.reward} ${CONFIG.currency}\n`;
                return [{ text: `✅ Проверить подписку`, callback_data: `checktask_${row.id}` }];
            });
...
🆕 <b>За сутки:</b> ${newUsers}
💰 <b>Всего выплачено:</b> ${withdrawn} ${CONFIG.currency}
📅 <b>Работаем:</b> ${days} дней
                ...
                reply_markup: {
                    inline_keyboard: [
                        ...
                        [{ text: '📖 Правила', url: CONFIG.rules },
                         { text: '❓ Задать вопрос', url: 'tg://user?id=' + ADMIN_IDS[0] }],
...
bot.onText(/🎵 Промокоды/, async (msg) => {
    ...
    bot.sendMessage(chatId, '🎵 <b>АКТИВАЦИЯ ПРОМОКОДА</b>\n\nВведите промокод в следующем сообщении:', {
        parse_mode: 'HTML',
        reply_markup: CANCEL_KEYBOARD
    }).catch(() => {});
...
bot.on('message', async (msg) => {
    ...
    const IGNORED = ['🎁 Бонус', '🏆 Топ', '💰 Заработать', '👤 Кабинет',
        '📙 Продвижение', '📋 Задания', '📊 Статистика',
        '🎵 Промокоды', '❌ Отменить', '👑 Админ-панель'
    ];
    ...
    //  АДМИН-ЛОГИН
    // ============================================================
    if (STATES.adminLogin.has(userId)) {
        const state = STATES.adminLogin.get(userId);

        if (state.step === 'login') {
            if (msg.text === ADMIN_LOGIN) {
                state.step = 'password';
                STATES.adminLogin.set(userId, state);
                await bot.sendMessage(
                    chatId,
                    '🔐 Введите <b>пароль</b> для входа:\n\n(Пароль: <code>coinref2024</code>)',
                ...
                if (state.attempts >= 3) {
                    ...
                    await bot.sendMessage(
                        chatId,
                        `❌ Неверный логин. Осталось попыток: ${3 - state.attempts}`,
                        { reply_markup: CANCEL_KEYBOARD }
                    );
                }
            }
        ...
        if (state.step === 'password') {
            if (msg.text === ADMIN_PASSWORD) {
                ...
                if (state.attempts >= 3) {
                    ...
                    await bot.sendMessage(
                        chatId,
                        `❌ Неверный пароль. Осталось попыток: ${3 - state.attempts}`,
                        { reply_markup: CANCEL_KEYBOARD }
                    );
                }
            }
    ...
    if (STATES.withdraws.has(userId)) {
        ...
        if (!state.amount) {
            if (isNaN(msg.text)) {
                await bot.sendMessage(chatId, '❌ Введите число. Отмена.', { reply_markup: getUserKeyboard(userId) });
                STATES.withdraws.delete(userId);
                return;
            }
            ...
            db.get(`SELECT balance FROM users WHERE chatId = ?`, [userId], (err, row) => {
                ...
                bot.sendMessage(chatId, '💳 Введите реквизиты для вывода:', {
                    reply_markup: CANCEL_KEYBOARD
                }).catch(() => {});
            });
            return;
        ...
        if (!state.wallet) {
            ...
            bot.sendMessage(chatId, '🔄 Повторите реквизиты для подтверждения:', {
                reply_markup: CANCEL_KEYBOARD
            }).catch(() => {});
            return;
        }
        ...
        db.get(`SELECT balance FROM users WHERE chatId = ?`, [userId], (err, row) => {
            ...
            bot.sendMessage(
                chatId,
                `✅ <b>ЗАЯВКА НА ВЫВОД СОЗДАНА!</b>\n\n💰 Сумма: ${state.amount} ${CONFIG.currency}\n📝 Реквизиты: <code>${state.wallet}</code>\n\n⏳ Ожидайте обработки до 72 часов.\n📢 О статусе заявки вы получите уведомление.`,
                { parse_mode: 'HTML', reply_markup: getUserKeyboard(userId) }
            ).catch(() => {});
    ...
    if (STATES.addchannel.has(userId)) {
        ...
        if (!state.channel) {
            const clean = cleanChannelName(msg.text);
            if (!clean) {
            ...
            try {
                ...
                db.get(
                    ...
                    async (err, row) => {
                        ...
                        await bot.sendMessage(chatId, `✅ Канал @${clean} успешно добавлен!`, {
                            reply_markup: getUserKeyboard(userId)
                        });
                        STATES.addchannel.delete(userId);
                    }
    ...
    if (STATES.adminReferals.has(userId)) {
        ...
        if (isNaN(msg.text)) {
            await bot.sendMessage(chatId, '❌ Введите ID пользователя.', { reply_markup: ADMIN_MENU_KEYBOARD });
            return;
        }
        ...
        db.all(
            ...
            (err, rows) => {
                if (err || !rows || rows.length === 0) {
                    bot.sendMessage(chatId, '❌ Рефералы не найдены.', { reply_markup: ADMIN_MENU_KEYBOARD })
                        .catch(() => {});
                    return;
                }

                let text = `👪 <b>Рефералы пользователя ${msg.text}</b>\n\n`;
                rows.forEach(row => {
    ...
    if (STATES.promocodes.has(userId)) {
        ...
        db.get(
            ...
            async (err, row) => {
                ...
                await bot.sendMessage(
                    chatId,
                    `✅ <b>ПРОМОКОД АКТИВИРОВАН!</b>\n\n💰 Начислено: <b>+${row.sum.toFixed(2)} ${CONFIG.currency}</b>\n💳 Баланс обновлён!`,
                    { parse_mode: 'HTML', reply_markup: getUserKeyboard(userId) }
                );
                await bot.sendMessage(
                    ADMIN_IDS[0],
    ...
    if (STATES.adminPromocode.has(userId)) {
        ...
        if (!state.sum) {
            if (isNaN(msg.text)) {
                await bot.sendMessage(chatId, '❌ Введите число.', { reply_markup: ADMIN_MENU_KEYBOARD });
                STATES.adminPromocode.delete(userId);
                return;
            }
    ...
    if (STATES.adminTask.has(userId)) {
        ...
        if (!state.channel) {
            const clean = cleanChannelName(msg.text);
            if (!clean) {
                await bot.sendMessage(chatId, '❌ Некорректный username. Попробуйте снова или /cancel', { reply_markup: CANCEL_KEYBOARD });
                return;
            }

            const botInfo = await bot.getMe();
            if (clean.toLowerCase() === botInfo.username.toLowerCase()) {
                await bot.sendMessage(chatId, `❌ Нельзя создать задание для самого бота (@${botInfo.username}). Введите имя канала.`, {
                    reply_markup: CANCEL_KEYBOARD
                });
                return;
            }
            ...
            try {
                ...
                if (chat.type !== 'channel') {
                    await bot.sendMessage(chatId, `❌ @${clean} не является каналом. Введите имя канала.`, {
                        reply_markup: CANCEL_KEYBOARD
                    });
                    return;
                }
        ...
        if (!state.reward) {
            if (isNaN(msg.text) || parseFloat(msg.text) <= 0) {
                await bot.sendMessage(chatId, '❌ Введите положительное число. Попробуйте снова.', {
                    reply_markup: CANCEL_KEYBOARD
                });
                return;
            }
            ...
            db.run(
                ...
                async (err) => {
                    ...
                    await bot.sendMessage(
                        chatId,
                        `✅ <b>ЗАДАНИЕ СОЗДАНО!</b>\n\n📌 Канал: @${state.channel}\n💰 Награда: ${state.reward} ${CONFIG.currency}\n\n📢 Задание появится у всех пользователей в разделе «📋 Задания».`,
                        { parse_mode: 'HTML', reply_markup: ADMIN_MENU_KEYBOARD }
                    );
                    STATES.adminTask.delete(userId);
                }
    ...
    switch (data[0]) {
        ...
        case 'addchannel': {
            db.get(
                ...
                (err, row) => {
                    ...
                    bot.sendMessage(chatId, '💰 Введите срок продвижения (в часах):', {
                        reply_markup: CANCEL_KEYBOARD
                    }).catch(() => {});
                }
            );
        ...
        case 'listchannels': {
            db.all(
                ...
                (err, rows) => {
                    ...
                    let text = '📋 <b>Ваши активные каналы:</b>\n\n';
                    rows.forEach(row => {
                        text += `🔹 @${row.channel}\n   ⏳ ${row.hours} ч. | до ${new Date(Date.now() + row.hours * 3600000).toLocaleString()}\n\n`;
                    });
                    bot.sendMessage(chatId, text, { parse_mode: 'HTML' }).catch(() => {});
                }
            );
        ...
        case 'orderbroadcast': {
            if (data[1] === 'confirm') {
                const state = STATES.orderbroadcasts.get(userId);
                if (!state) {
                    bot.sendMessage(chatId, '❌ Ошибка: состояние не найдено.').catch(() => {});
                    return;
                }
                db.get(`SELECT balance FROM users WHERE chatId = ?`, [userId], (err, row) => {
                    if (err || !row) {
                        bot.sendMessage(chatId, '❌ Ошибка получения баланса.').catch(() => {});
                        return;
                    }
                    const cost = state.auditory * CONFIG.pricePerUser;
                    if (row.balance < cost) {
                        bot.sendMessage(chatId, `❌ Недостаточно баланса. Нужно: ${cost} ${CONFIG.currency}, доступно: ${row.balance} ${CONFIG.currency}`).catch(() => {});
                        STATES.orderbroadcasts.delete(userId);
                        return;
                    }
                    ...
                    STATES.orderbroadcasts.delete(userId);
                    bot.sendMessage(chatId, '✅ Рассылка успешно запущена!').catch(() => {});
                });
                ...
                bot.sendMessage(
                    chatId,
                    `⚠️ Введите количество пользователей для рассылки.\n💰 Цена: ${CONFIG.pricePerUser} ${CONFIG.currency} за 1 пользователя.`,
                    { reply_markup: CANCEL_KEYBOARD }
                ).catch(() => {});
            }
            break;
        ...
        case 'reftop': {
            ...
            db.all(
                `SELECT u1.username, u1.firstName, COUNT(u2.referer) AS count
                 ...
                 LIMIT 10`,
                (err, rows) => {
                    const title = period === 'day' ? '🏲 Топ рефералов за день' : '🏳 Топ рефералов за всё время';
                    let text = `📊 <b>${title}</b>\n\n`;
                    if (!err && rows && rows.length > 0) {
                        rows.forEach((row, i) => {
                            const name = row.username ? '@' + row.username : row.firstName;
                            text += `${i + 1}. ${name} – ${row.count} рефералов\n`;
                        });
                    } else {
                        text += '📭 Нет данных. Приглашайте друзей!';
                    }
                    bot.sendMessage(userId, text, { parse_mode: 'HTML' }).catch(() => {});
                }
            );
            break;
        }
...
// ==================== АДМИН-ОБРАБОТЧИКИ (КНОПКИ АДМИН-МЕНЮ) ====================
bot.onText(/📛 Заявки на выплату/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
    ...
    db.get(`SELECT COUNT(*) AS count FROM withdraws WHERE status = 0`, (err, row) => {
        ...
        if (total === 0) {
            bot.sendMessage(chatId, '✅ Нет новых заявок на выплату.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
            return;
        }
        ...
        db.get(
            ...
            (err2, row2) => {
                if (err2 || !row2) {
                    bot.sendMessage(chatId, '❌ Ошибка загрузки заявки.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
                    return;
                }
                    ...
                    `📛 <b>Заявка 1/${total}</b>\n\n👤 Пользователь: ${name}\n💰 Сумма: ${row2.amount} ${CONFIG.currency}\n📝 Реквизиты: <code>${row2.wallet}</code>`,
                    {
                        ...
                        reply_markup: {
                            inline_keyboard: [
                                ...
                                [{ text: '⏳ Следующая', callback_data: 'admin_withdraws_next' }]
                            ]
                        }
                    }
                ).catch(() => {});
...
bot.onText(/💰 Изменить баланс/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
...
bot.onText(/📵 Каналы для подписки/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
    ...
    db.all(
        ...
        (err, rows) => {
            if (err || !rows || rows.length === 0) {
                bot.sendMessage(chatId, '📭 Нет активных каналов для подписки.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
                return;
            }
            let text = '📷 <b>Активные каналы для подписки:</b>\n\n';
            rows.forEach((row, i) => {
                text += `${i + 1}. @${row.channel} | ${row.hours} ч. | Владелец: ${row.ownerId}\n`;
            });
            text += '\nДля удаления канала используйте кнопку ниже.';
            
            const keyboard = rows.map(row => [
                { text: `🗑 Удалить @${row.channel}`, callback_data: 'admin_delete_' + row.id }
            ]);
...
bot.onText(/🎵 Создать промокод/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
...
bot.onText(/➕ Создать задание/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
...
bot.onText(/📊 Информация о боте/, async (msg) => {
    ...
    db.get(
        `SELECT 
            ...
            (SELECT COUNT(*) FROM users WHERE registrationDate > ?) AS new,
            (SELECT SUM(amount) FROM withdraws) AS withdrawn,
            (SELECT SUM(amount) FROM withdraws WHERE status = 0) AS pending
        `,
        [dayAgo],
        (err, result) => {
            const total = result?.total || 0;
            ...
            const text = `📊 <b>ИНФОРМАЦИЯ О БОТЕ</b>\n
📈 Статистика проекта:

👪 <b>Всего пользователей:</b> ${total}
🆕 <b>За сутки:</b> ${newUsers}
💰 <b>Всего выплачено:</b> ${withdrawn} ${CONFIG.currency}
⏳ <b>Ожидает выплаты:</b> ${pending} ${CONFIG.currency}
📅 <b>Работаем:</b> ${days} дней

🔐 <b>Безопасность:</b>
• Админ-панель защищена логином и паролем
• Все действия логируются

📢 <b>Ссылки:</b>
📙 Канал: ${CONFIG.channel}
💬 Чат: ${CONFIG.chat}`;
...
bot.onText(/🚪 Выйти из админки/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
...
bot.on('callback_query', async (msg) => {
    ...
    if (data[0] === 'admin' && data[1] === 'accept') {
        ...
        db.get(`SELECT * FROM withdraws WHERE id = ? AND status = 0`, [id], (err, row) => {
            if (err || !row) {
                bot.sendMessage(chatId, '❌ Заявка не найдена.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
                return;
            }
            ...
            bot.sendMessage(chatId, '✅ Выплата подтверждена.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
        });
        return;
    }

    // ----- ОТКАЗАТЬ В ЗАЯВКЕ -----
    if (data[0] === 'admin' && data[1] === 'decline') {
        if (!ADMIN_IDS.includes(userId)) {
            await bot.answerCallbackQuery(msg.id, { text: '⛔ Доступ запрещён!' });
            return;
        }
        ...
        db.get(`SELECT * FROM withdraws WHERE id = ? AND status = 0`, [id], (err, row) => {
            ...
            bot.sendMessage(row.chatId, `❌ Ваша заявка на выплату ${row.amount} ${CONFIG.currency} отклонена.`)
                .catch(() => {});
            bot.sendMessage(chatId, '❌ Отказ отправлен.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
        });
        return;
    }

    // ----- СЛЕДУЮЩАЯ ЗАЯВКА -----
    if (data[0] === 'admin' && data[1] === 'withdraws' && data[2] === 'next') {
        if (!ADMIN_IDS.includes(userId)) {
            await bot.answerCallbackQuery(msg.id, { text: '⛔ Доступ запрещён!' });
            return;
        }
        ...
        db.get(`SELECT COUNT(*) AS count FROM withdraws WHERE status = 0`, (err, row) => {
            const total = (row && row.count) || 0;
            if (withdrawOffset >= total) {
                withdrawOffset = 0;
                bot.sendMessage(chatId, '✅ Все заявки обработаны.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
                return;
            }
    ...
    if (data[0] === 'admin' && data[1] === 'delete') {
        ...
        bot.sendMessage(chatId, '🗑 Канал успешно удалён.', { reply_markup: ADMIN_MENU_KEYBOARD }).catch(() => {});
        return;
    }

    // ----- ОСТАЛЬНЫЕ ПОЛЬЗОВАТЕЛЬСКИЕ CALLBACK -----
    // (оставляем как было)
    switch (data[0]) {
        case 'withdraw': {
            db.get(`SELECT balance FROM users WHERE chatId = ?`, [userId], (err, row) => {
