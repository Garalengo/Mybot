 ...
 * Версия: 9.0 (РАБОЧАЯ + ЗАЩИТА ПАРОЛЕМ)
 * ============================================================
 */
...
// Клавиатура с кнопкой входа в админку (для админов)
...
bot.onText(/👑 Вход в админку/, async (msg) => {
    const userId = msg.from.id;
    const chatId = msg.chat.id;
    ...
    STATES.adminLogin.set(userId, { attempts: 0 });
    await bot.sendMessage(
        chatId,
...
bot.on('message', async (msg) => {
    ...
    const IGNORED = ['🎁 Бонус', '🏆 Топ', '💰 Заработать', '👤 Кабинет',
        ...
        '🎵 Промокоды', '❌ Отменить', '👑 Вход в админку'
    ];
    if (msg.text.startsWith('/') || IGNORED.includes(msg.text)) return;

    // ============================================================
    //  АДМИН-ЛОГИН (ПРОВЕРКА ПАРОЛЯ)
    // ============================================================
